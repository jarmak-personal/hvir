...
| Deduplication and resolution | A main window emits on 30-second rollover or on `user-stop`, `timeout`, `backgrounded`, `api-unavailable`, or `renderer-revoked`. There is no automatic recovery. Repeated Stop/Delete is harmless, and a new run deletes the prior retained run. |
