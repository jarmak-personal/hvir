...
- [ ] **Make dirty buffers lossless across ordinary navigation.** Reopening an existing
...
- [ ] **Repair the SSH auth error contract.** Recoverable `ssh2` agent/signing errors must
...
- [ ] **Make the Git worker broker a real main-side boundary.** Replace raw permissive Git
...
- [ ] **Open deleted and historical paths as diffs.** Git confinement must permit a missing
...
- [ ] **Report every untracked file honestly.** Use porcelain `--untracked-files=all` so a
      new directory is not an unopenable pseudo-file. Show real additions or omit counts;
      never present fabricated `+0 -0` statistics.
- [ ] **Keep diff content and labels truthful.** Unsaved working content belongs only in
...
- [ ] Gate all client close/error effects by client identity or connection generation.
...
- [ ] Report remote PTY exit exactly once from either SSH `exit` or `close`. A channel that
...
- [ ] Make pending PTY spawn part of session lifecycle. Disconnect, project replacement,
...
- [ ] Separate synthetic tree-liveness pulses from Git invalidation. An idle SSH project must
...
- [ ] Close remote cleanup edge cases: root `/` cache invalidation, inotify move/remove and
...
- [ ] Model branch-point availability and selected base explicitly. A shallow/no-merge-base,
...
- [ ] Support projects opened below their containing repository root without authorizing the
      parent tree. Keep Git `-C` inside the selected root and scope commands with repository
      prefix/pathspec information.
- [ ] Reset GitPanel state on root/disconnect and generation-key every Changes, History, and
...
- [ ] Present unborn and non-repository projects as normal contained states rather than raw
      fatal Git output; never leave the previous project's Changes visible beneath a new root.
- [ ] Replace increasing `git log --skip=N` rescans with stable continuation/streaming and
...
- [ ] Support legal unusual repositories: parse tab-containing NUL-delimited numstat paths
      without truncation and accept SHA-1 or SHA-256 object IDs throughout detail/blame/diff.
- [ ] Keep large blame/Changes/detail rendering bounded: transfer compact blame runs/shared
      commit metadata and materialize only visible gutter/list rows.

## P1/P2 — integrated UX quality

- [ ] Add a Restart action for an exited plain shell.
- [ ] Make safe in-project symlinked files and directories browsable while retaining canonical
      confinement; clearly mark links rather than rendering them as disabled mystery rows.
- [ ] Preserve independent scroll position for source, rendered Markdown, diff, JSON, and YAML
      across tab/mode changes.
- [ ] Make session/auth dialogs keyboard-modal (`role=dialog`, `aria-modal`, initial focus,
      Escape, and focus containment) and give ARIA trees the expected arrow-key behavior.
- [ ] Disambiguate duplicate Git basenames with compact repository-relative context. Give
      loading, clean, empty-history, unborn, non-repository, stale, and operational-error states
      distinct presentations.
- [ ] Route repository-relative Markdown images through the owning `ProjectHost` or show an
      explicit unavailable state; remote documentation must not silently lose screenshots.
- [ ] Protect paint from very large source files. Do not synchronously construct a 64 MiB
...
- [ ] Focused unit/integration coverage: recoverable agent error, delayed old close, capability
...
- [ ] Full `npm run verify`, production Electron smoke, and real SSH regression pass remain
      green. Phase 4/5 acceptance boxes are updated only after their real-host criteria pass.
