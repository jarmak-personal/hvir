...
### Tabs
- [x] Tab strip over the viewer: open from tree (single-click preview-tab, double-click
      pinned — VSCode convention), close, reorder, dirty indicator.
- [x] Per-tab state (view mode, scroll position) survives tab switches; open-tab set
      survives app restart.

### View modes (ADR-007)
- [x] Per-tab mode: `rendered | source | diff`. Visible segmented control in the tab
      header + one keybinding cycling modes. Mode is sticky per tab.
- [x] Default inference: markdown/mermaid/HTML/JSON open **rendered**; everything else
...
- [x] **Rendered: markdown** — markdown-it + Shiki for fenced code. Mermaid blocks
      render as diagrams (verify current mermaid API; render off-thread or lazily).
- [x] **Rendered: HTML** — sandboxed iframe, behaving like a browser opened the file.
...
- [x] **Rendered: JSON** — pretty-printed, collapsible tree.
- [x] **Diff** — this file vs a base, with a small **base selector**: working tree vs
...
- [x] Git module in a utility process, shelling out to system `git` **through
...
- [x] CodeMirror merge/diff view rendering the two versions.

### Live-reload & minor edit
- [x] Open tabs subscribe to `ProjectHost.watch`: file changed externally → clean tab
      reloads in place (preserving scroll); dirty tab shows a conflict indicator, never
      silently clobbers either side.
- [x] Minor edit + save: source mode is editable, `Ctrl+S` writes via
      `ProjectHost.writeFile`. **This is the guardrail (§2): no format-on-save, no
      multi-file operations, no LSP anything.**

## Acceptance criteria
- [x] A markdown file opens rendered with highlighted code and a working mermaid
      diagram; one keystroke flips it to source; edit + save works; the rendered view
      updates.
- [x] An HTML file renders like a browser page; its JS cannot reach node, navigate the
      app, or open windows (write a test page that tries).
- [x] Diff mode shows a dirty file vs HEAD and vs branch-point via the base selector.
- [x] `touch`/append to an open file from a terminal → tab live-reloads without focus
      change or scroll jump.
- [x] All mode switches feel instant; rendering heavy files never stalls the UI.
- [x] Status table updated.

Verified 2026-07-12 with seam checks, lint, both TypeScript targets, 31 unit tests,
the production build, and an Electron/Xvfb smoke. The smoke covers rendered Markdown
(Shiki + Mermaid, including scroll regression), the custom-protocol adversarial HTML
sandbox fixture, worker-backed lazy JSON, source edit + `Ctrl+S`, clean tab live-reload
with scroll preservation, and working-tree/HEAD/branch-point merge views.
