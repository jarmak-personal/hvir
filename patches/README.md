...
terminal buffer. The patch also accepts an explicit WASM URL and exposes soft-wrap state for
scrollback rows so hvir's terminal link provider can preserve logical lines after they leave the
active screen.

`src/renderer/src/terminal/ghostty-vt.wasm` is built from the Ghostty submodule at
`5714ed07a1012573261b7b7e3ed2add9c1504496` in `coder/ghostty-web` tag `v0.4.0`.
Apply that release's `patches/ghostty-wasm-api.patch`, then
