...
export class SshManagedDirectory implements ManagedDirectoryPort {
  ...
  private async run(
    ...
    // A transport can close during an awaited write; attach the rejection handler immediately.
    void exited.catch(() => {})
    try {
      ...
      for (let offset = 0; offset < header.length;) {
        ...
        await stream.write(header.slice(offset, end))
        offset = end
      }
      if (bytes) {
      ...
      if (!submitted && request.operation === 'inspect' && signal.aborted)
        throw signal.reason
      throw uncertain()
    } finally {
      clearTimeout(timer)
