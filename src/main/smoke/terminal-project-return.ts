...
export async function verifyTerminalProjectReturn(
  ...
  const terminal = supervisor
    ...
    .find(
      (candidate) =>
        candidate.ownerId === win.webContents.id &&
        (terminalId === undefined || candidate.id === terminalId),
    )
  if (!terminal) throw new Error('project return has no live terminal')
  if (terminalId) await selectTerminal(win, terminal.id)
  supervisor.write(
    terminal.id,
    terminal.ownerId,
...
async function selectTerminal(win: BrowserWindow, terminalId: string): Promise<void> {
  await withProjectReturnTimeout(
    win.webContents.executeJavaScript(`
      new Promise((resolve, reject) => {
        const sessionId = ${JSON.stringify(terminalId)};
        const deadline = Date.now() + 5000;
        const poll = () => {
          const surface = document.querySelector(
            '.terminal-surface[data-terminal-session="' + CSS.escape(sessionId) + '"]'
          );
          if (surface?.classList.contains('active')) return resolve(true);
          const button = document.querySelector(
            '.terminal-list-main[data-terminal-session="' + CSS.escape(sessionId) + '"]'
          );
          if (button instanceof HTMLButtonElement) button.click();
          if (Date.now() > deadline) {
            return reject(new Error('project return terminal did not become active'));
          }
          setTimeout(poll, 25);
        };
        poll();
