...
export function useTerminalPaneController(options: TerminalPaneControllerOptions) {
  const {
    sessionId,
    profileId,
    launchRevision,
  ...
  if (
    workspaceRootRef.current.hostId !== options.cwd.hostId ||
    workspaceRootRef.current.path !== options.cwd.path
  ) {
    workspaceRootRef.current = options.cwd
  }
  ...
  const focus = useCallback((): void => handlersRef.current.onFocus(), [])

  return { workspaceRoot, containerRef, title, status, exited, restart, focus }
}
