...
if [[ -n "$configured_hooks_path" && "$configured_hooks_path" != ".githooks" ]]; then
  ...
  echo "Integrate .githooks/pre-push there manually if you want hvir's local smoke hook." >&2
  exit 1
fi

git config --local core.hooksPath .githooks
echo "Installed hvir Git hooks from .githooks/."
echo "Pushes now run the local-platform Electron smoke test; use git push --no-verify to bypass it."
