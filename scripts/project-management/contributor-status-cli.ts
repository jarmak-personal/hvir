...
export const CONTRIBUTOR_STATUS_HELP = `Usage: npm run project:status -- --issue N [--pr N] [--json]
       npm run project:status -- --issue N --capture codex|claude-code [--apply]
...
One session belongs to one issue. Totals are observed estimates since migration, not exact effort.
